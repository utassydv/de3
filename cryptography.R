...
bytes.to.encode = charToRaw("Are you staying in Budapest during the curfew? :) ")
...
#loading in pair's pub key
pub_xingi.pem.loaded <- scan("id_rsa_xingi.pub", what='list', sep='\n') # Load

# Extract the key objects from the PEM file
pub_xingi.key.loaded <- PKI.load.key(pub_xingi.pem.loaded) 


# Let's encrypt and decrypt, again!
encrypted.again <- PKI.encrypt(charToRaw("Are you staying in Budapest during the curfew? :) "), pub_xingi.key.loaded)


# Create a connection object to write the binary file using mode "wb".
write.filename = file("davids_message.dat", "wb")

# Write the column names of the data frame to the connection object.
writeBin(encrypted.again, write.filename)

close(write.filename)

read.binfile <- file("davids_message.dat", "rb")
...
decrypted.again <- rawToChar(PKI.decrypt(read.binfile, prv.key))
print(decrypted.again)
